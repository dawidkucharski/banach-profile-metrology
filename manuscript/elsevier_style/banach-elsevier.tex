...
Several limitations should be acknowledged. First, the uncertainty analysis is restricted to within-run Type~A repeatability from five sweeps in a single continuous rotation, complemented by the indicative type-B budget of Sec.~\ref{sec:typeB}, which now includes a measured drift and noise-floor bound from an independent static stability session. The small $n=5$ limits statistical power (relative differences below $\sim30\%$ cannot be resolved for the FN~111), and between-run reproducibility of the rotating measurement (independent setups, thermal cycles, physical re-centring of the artefact) has not been assessed; the reported expanded uncertainties should be interpreted as lower bounds, and the form-removal systematic (bounded at $\leq41$~nm) is carried outside the quadrature combination.
