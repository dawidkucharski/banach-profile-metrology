...
\caption{Full-resolution radial EFM reconstruction from raw interferograms (LSCI + Gaussian low-pass, 50\% at 15~UPR, ISO~16610-21:2025). Uncertainties are 95\% expanded within-run repeatability estimates from five sweeps ($n=5$, $t_{0.975,4}=2.776$). Units: nm for all descriptors except BV density and $W^{1,2}$ gradient (nm\,sample$^{-1}$). Certified $RONt$: FN~111 $244 \pm 52$~nm (Hommel Roundscan~535, $n=104$); FN~101 $41.94$~$\mu$m (form tester, $n=12$). The FN~101 certificate does not report an expanded uncertainty; therefore only the percentage difference is reported for this artefact rather than an $E_n$ score.}
