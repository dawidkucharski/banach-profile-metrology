...
The numerical analysis of fringe patterns has its own methodological lineage, which frames the phase-decoding route used here. Takeda et al.\ introduced the Fourier-transform method of fringe-pattern analysis, in which a spatial carrier allows the phase to be recovered from a single interferogram without temporal phase shifting~\cite{Takeda1982}. Kemao extended this concept with the two-dimensional windowed Fourier transform, improving robustness to local fringe-quality variations at the cost of window-parameter selection~\cite{Kemao2007}. More recently, Feng et al.\ applied deep learning to single-frame fringe analysis, training convolutional networks to estimate the intermediate quantities of the arctangent phase calculation and reporting accuracy approaching multi-frame phase shifting~\cite{Feng2019dl}. Galaktionov and Toporovsky proposed a multi-stage fringe-map reconstruction algorithm for fibre-end surface analysis in non-phase-shifting interferometry, showing that geometric fringe processing remains competitive where phase-shifting hardware is unavailable~\cite{Galaktionov2026}. The radial excess-fraction method used in the present work belongs to this single-frame, non-phase-shifting family: each interferogram is decoded independently through the geometry of its circular fringes, which suits continuous-rotation acquisition where temporal phase stepping is impractical.
...
\item \textbf{Report in angular units.} The BV density is reported as total variation per unit angle, $\mathrm{BV}_\theta = TV(p)/\Theta$ in nm\,deg$^{-1}$ ($\Theta=360^\circ$), and the gradient content as the RMS angular gradient $g_{\mathrm{rms}} = [\frac{1}{N-1}\sum_j ((p_{j+1}-p_j)/\Delta\alpha)^2]^{1/2}$ in nm\,deg$^{-1}$, together with the filter cutoff.
\end{enumerate}

For the present data, the per-sample values in Table~\ref{tab:metric_phase_reconstruction} convert as follows. FN~111 ($\Delta\alpha=0.083^\circ$): BV density $0.263$~nm\,sample$^{-1}$ $\rightarrow$ $3.2$~nm\,deg$^{-1}$; RMS gradient $0.310$~nm\,sample$^{-1}$ $\rightarrow$ $3.7$~nm\,deg$^{-1}$. FN~101 ($\Delta\alpha=0.020^\circ$): BV density $6.58$~nm\,sample$^{-1}$ $\rightarrow$ $329$~nm\,deg$^{-1}$; RMS gradient $8.76$~nm\,sample$^{-1}$ $\rightarrow$ $438$~nm\,deg$^{-1}$, all at the 15-UPR cutoff. Because the conversion is a fixed multiplicative factor for a given acquisition, relative uncertainties and relative defect responses are identical in either unit system. Adoption of gradient descriptors in reference software for roundness evaluation would require exactly this pair of declarations — filter cutoff and angular units — which parallels the established practice of declaring the filter for $RONt$.
...
\midrule
Form removal (lima\c{c}on second order) & $a^2/(2R)$, $a=15$--$35~\mu$m & $\leq41$~nm (systematic bound) \\
\bottomrule
\end{tabular}
\end{table}

The quadrature combination is dominated by the type-A term, so the reported $U_{95}$ values are essentially unchanged by the enumerated random type-B terms; the budget nevertheless shows that the leading unquantified risks are the form-removal systematic and the untested between-run effects, not the enumerated instrument terms.
...
Several limitations should be acknowledged. First, the uncertainty analysis is restricted to within-run Type~A repeatability from five sweeps in a single continuous rotation, complemented by the indicative type-B budget of Sec.~\ref{sec:typeB}. The small $n=5$ limits statistical power (relative differences below $\sim30\%$ cannot be resolved for the FN~111), and between-run reproducibility (independent setups, thermal cycles, physical re-centring of the artefact) has not been assessed; the reported expanded uncertainties should be interpreted as lower bounds, and the form-removal systematic (bounded at $\leq41$~nm) is carried outside the quadrature combination.
