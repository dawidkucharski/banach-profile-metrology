...
%DIF ADD banach-elsevier.tex                       Sat Aug 15 08:18:25 2026
...
\cite{Brodmann1983,Brodmann1986,Fan1994,Hertzsch2002,Kucharski_D._A_2020}}\hskip0pt%DIFAUXCMD
...
\cite{Brodmann1983} }\hskip0pt%DIFAUXCMD
...
; and Kucharski and Zdunek reported a low-cost scatterometry setup achieving nanometric precision in fast roughness measurement~\mbox{%DIFAUXCMD
...
. Kemao extended this concept with the two-dimensional windowed Fourier transform, improving robustness to local fringe-quality variations at the cost of window-parameter selection~\mbox{%DIFAUXCMD
\cite{Kemao2007}}\hskip0pt%DIFAUXCMD
. More recently, Feng et al.\ applied deep learning to single-frame fringe analysis, training convolutional networks to estimate the intermediate quantities of the arctangent phase calculation and reporting accuracy approaching multi-frame phase shifting~\mbox{%DIFAUXCMD
\cite{Feng2019dl}}\hskip0pt%DIFAUXCMD
. Galaktionov and Toporovsky proposed a multi-stage fringe-map reconstruction algorithm for fibre-end surface analysis in non-phase-shifting interferometry, showing that geometric fringe processing remains competitive where phase-shifting hardware is unavailable~\mbox{%DIFAUXCMD
...
\midrule
\DIFaddFL{Form removal (lima\c{c}on second order) }& \DIFaddFL{$a^2/(2R)$, $a=15$--$35~\mu$m }& \DIFaddFL{$\leq41$~nm (systematic bound) }\\
\bottomrule
\end{tabular}
\end{table}

\DIFadd{The quadrature combination is dominated by the type-A term, so the reported $U_{95}$ values are essentially unchanged by the enumerated random type-B terms; the budget nevertheless shows that the leading unquantified risks are the form-removal systematic and the untested between-run effects, not the enumerated instrument terms.
}
...
Several limitations should be acknowledged. First, the uncertainty analysis is restricted to within-run Type~A repeatability from five sweeps in a single continuous rotation\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, complemented by the indicative type-B budget of Sec.~\ref{sec:typeB}. }\DIFaddend The small $n=5$ limits statistical power (relative differences below $\sim30\%$ cannot be resolved \DIFaddbegin \DIFadd{for the FN~111}\DIFaddend ), and between-run reproducibility (independent setups, thermal cycles, \DIFaddbegin \DIFadd{physical }\DIFaddend re-centring \DIFaddbegin \DIFadd{of the artefact}\DIFaddend ) has not been assessed\DIFdelbegin \DIFdel{. A full GUM-compliant uncertainty budget, incorporating type-B evaluations of laser wavelength stability, refractive-index variations, Abbe error, camera nonlinearity, and form-removal uncertainty, is beyond the scope of this study}\DIFdelend ; the reported expanded uncertainties should be interpreted as lower bounds\DIFaddbegin \DIFadd{, and the form-removal systematic (bounded at $\leq41$~nm) is carried outside the quadrature combination}\DIFaddend .
